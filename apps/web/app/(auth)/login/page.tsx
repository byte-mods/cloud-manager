...
export default function LoginPage() {
  ...
  const [isMfaSubmitting, setIsMfaSubmitting] = React.useState(false);
  ...
  async function onMfaSubmit(values: MfaFormValues) {
    ...
    try {
      const result = await signIn('credentials', {
        email: '', // Email is stored from first login attempt
        password: '', // Password is not needed for MFA
        mfaCode: values.code,
        redirect: false,
      });

      if (result?.error) {
        setServerError('Invalid MFA code. Please try again.');
      ...
      setServerError(err?.message ?? 'Invalid MFA code. Please try again.');
    } finally {
      setIsMfaSubmitting(false);
    }
  }
  ...
  function handleOAuth(provider: 'google' | 'github' | 'azure-ad') {

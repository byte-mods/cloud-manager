mod config;
mod error;
mod handlers;
mod models;
mod providers;
mod store;
mod traits;

use std::sync::Arc;

use actix_cors::Cors;
use actix_web::{web, App, HttpServer};
use tracing_subscriber::EnvFilter;

use crate::config::AppConfig;
use crate::providers::ProviderContext;

#[actix_web::main]
async fn main() -> std::io::Result<()> {
    dotenv::dotenv().ok();

    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::from_default_env()
                .add_directive("analytics_service=info".parse().unwrap()),
        )
        .init();

    let config = AppConfig::from_env().expect("Failed to load configuration");
    let port = config.server_port;
    let flags = cloud_common::FeatureFlags::from_env();

    // Initialize embedded SurrealDB for persistent storage
    let db = cloud_common::Database::new("./data/analytics").await.unwrap();
    db.init_schema().await.ok();
    store::seed_if_empty(&db).await;
    tracing::info!("SurrealDB initialized and seeded for analytics-service");

    let db_arc = Arc::new(db);

    // Initialize real cloud credentials if not in mock mode
    let (credentials, cache) = if flags.use_real_sdk() {
        tracing::info!("Real SDK mode enabled — initializing cloud credentials");

        let creds = Arc::new(cloud_common::CredentialManager::new().await);
        let available = creds.available_providers();
        tracing::info!(providers = ?available, "Cloud credentials loaded");

        let redis_url =
            std::env::var("REDIS_URL").unwrap_or_else(|_| "redis://127.0.0.1:6379".to_owned());
        let cache = cloud_common::RedisCache::new(&redis_url, "analytics-service")
            .await
            .map(Arc::new)
            .ok();

        if cache.is_none() {
            tracing::warn!("Redis not available — caching disabled");
        }

        (Some(creds), cache)
    } else {
        tracing::info!("Mock mode enabled (CLOUD_USE_MOCK_DATA=true) — using SurrealDB data");
        (None, None)
    };

    let provider_ctx = Arc::new(ProviderContext {
        db: db_arc.clone(),
        credentials,
        cache,
        flags,
    });

    // Create the query engine provider once and share it across workers
    let query_provider = providers::get_query_engine_provider(&provider_ctx);

    let db_data = web::Data::from(db_arc);

    tracing::info!("Starting Analytics Service on port {port}");

    HttpServer::new(move || {
        let cors = Cors::default()
            .allow_any_origin()
            .allow_any_method()
            .allow_any_header()
            .max_age(3600);

        App::new()
            .wrap(cors)
            .app_data(web::Data::from(query_provider.clone()))
            .app_data(db_data.clone())
            .route("/health", web::get().to(health))
            // Overview
            .route(
                "/api/v1/analytics/overview",
                web::get().to(handlers::overview::get_overview),
            )
            // Query engines
            .route(
                "/api/v1/analytics/query-engines",
                web::get().to(handlers::query_engines::list_query_engines),
            )
            // Query execution
            .route(
                "/api/v1/analytics/query",
                web::post().to(handlers::query::execute_query),
            )
            // Visualizations
            .route(
                "/api/v1/analytics/visualizations",
                web::get().to(handlers::visualizations::list_visualizations),
            )
            .route(
                "/api/v1/analytics/visualizations",
                web::post().to(handlers::visualizations::create_visualization),
            )
            // Reports
            .route(
                "/api/v1/analytics/reports",
                web::get().to(handlers::reports::list_reports),
            )
            .route(
                "/api/v1/analytics/reports",
                web::post().to(handlers::reports::create_report),
            )
            .route(
                "/api/v1/analytics/reports/{id}",
                web::get().to(handlers::reports::get_report),
            )
            // Search
            .route(
                "/api/v1/analytics/search",
                web::get().to(handlers::search::get_search_status),
            )
    })
    .bind(("0.0.0.0", port))?
    .run()
    .await
}

async fn health() -> actix_web::HttpResponse {
    actix_web::HttpResponse::Ok().json(serde_json::json!({
        "status": "healthy",
        "service": "analytics-service",
        "timestamp": chrono::Utc::now().to_rfc3339(),
    }))
}
